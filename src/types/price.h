...
public:
    using Price = QString;
    using PriceLevels = std::array<Price, static_cast<size_t>(PriceLevel::Value::PriceLevelSize)>;
    using Prices = std::array<PriceLevels, static_cast<size_t>(PriceType::Value::PriceTypeSize)>;
    ...
    static MarketPrice::PriceLevels pricesFromJson(const QString &name, QJsonObject &json);
    static MarketPrice::PriceLevels pricesFromJsonCardMarket(const QString &name, QJsonObject &json) ;
