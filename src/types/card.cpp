...
void
Card::setName(const QString& value)
{
  m_name = value;
  emit dataChanged();
}
...
QString
Card::supertype() const
{
  return m_supertype;
}

QString
Card::smallImageUrl() const
{
  return m_smallImageUrl;
}

QString
Card::largeImageUrl() const
{
...
int
Card::rulesSize() const
{
  return m_additionalRules.size();
}

QString
Card::rule(int index) const
{
...
int
Card::abilitiesSize() const
{
  return m_abilities.size();
}

Ability*
Card::ability(int index)
{
...
int
Card::attackSize() const
{
  return m_attacks.size();
}

Attack*
Card::attack(int index)
{
...
QString
Card::prices(const QString& market_name,
...
QString
Card::priceUrl(const QString& market_name)
{
  auto market_prices = m_prices[market_name];
  if (market_prices) {
...
bool
Card::hasRetreatCost() const
{
  return m_hasRetreatCost;
}

int
Card::retreatCost() const
{
...
QString
Card::weaknessValue() const
{
  return m_weaknessValue;
}

QString
Card::resistances() const
{
  return m_resistances;
}

QString
Card::resistancesValue() const
{
  return m_resistancesValue;
}

int
Card::nationalPokedexNumber() const
{
  return m_nationalPokedexNumber;
}

void
Card::setNationalPokedexNumber(int nationalPokedexNumber)
{
...
void
Card::setCardNumber(QString number)
{
  card_number_ = number;
}

void
Card::fromJson(QJsonObject& json)
{
  ...
  auto nationalPokedexNumberJsonArray =
    json["nationalPokedexNumbers"].toArray();
...
int
CardList::counter(int index) const
{
  if (index >= m_cards.size()) {
    return 0;
  }
...
int
CardList::counter(QString id) const
{
  auto row = m_idToRow.find(id);
  if (row != m_idToRow.end()) {
...
int
CardList::index(QString id) const
{
  auto row = m_idToRow.find(id);
  if (row != m_idToRow.end()) {
    return row->second;
...
void
CardList::append(const CardPtr& card, int counter)
{
  if (card == nullptr) {
    return;
  }
...
void
CardList::appendList(CardListPtr& cards)
{
  if (!cards) {
    return;
  }
...
bool
CardList::exist(QString id) const
{
