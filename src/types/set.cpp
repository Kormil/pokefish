...
int
SetList::index(QString id) const
{
  auto row = m_idToRow.find(id);
  if (row != m_idToRow.end()) {
    return row->second;
...
void
SetList::append(const SetPtr& set)
{
  if (set == nullptr) {
    return;
  }
...
void
SetList::appendList(SetListPtr& sets)
{
