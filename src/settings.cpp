...
#define NETWORK_ALWAYS_LARGE_IMAGES_SETTINGS_PATH                              \
  QStringLiteral("network/always_large_images")
...
Settings::Settings()
{
  ...
  m_settings =
    std::make_unique<QSettings>(settings_path, QSettings::NativeFormat);
}
...
Settings::migratedb()
{
  ...
  if (!db_dir.exists()) {
    QDir oldDbDir(
      ...
      QCoreApplication::applicationName() + "/QML/OfflineStorage/Databases/");

    db_dir.mkpath(".");

    const QStringList db_files =
      oldDbDir.entryList({ "*.sqlite", "*.ini" }, QDir::Files);
...
int
Settings::sortCardsBy() const
{
  return m_settings->value(CARDS_SORT_BY_SETTINGS_PATH, 0).toInt();
}

void
Settings::setSortCardsBy(int value)
{
...
bool
Settings::alwaysLargeImages() const
{
  return m_settings->value(NETWORK_ALWAYS_LARGE_IMAGES_SETTINGS_PATH, false)
    .toBool();
}

void
Settings::setAlwaysLargeImages(bool value)
{
...
QObject*
Settings::instance(QQmlEngine* engine, QJSEngine* scriptEngine)
{
  Q_UNUSED(engine)
...
Settings::bindToQml()
{
  qmlRegisterSingletonType<Settings>(
    "Settings", 1, 0, "Settings", Settings::instance);
}
