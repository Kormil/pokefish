...
DeckExporter::DeckExporter() : QObject(nullptr)
{

}

void DeckExporter::bindToQml()
{
    qmlRegisterType<DeckExporter>("DeckExporter", 1, 0, "DeckExporter");
}

void DeckExporter::setModelsManager(ModelsManager* modelsManager) {
    models_manager_ = modelsManager;
}

void DeckExporter::reset() {
    pokemons_.clear();
    energies_.clear();
    trainers_.clear();
}

int DeckExporter::loadData(QAbstractListModel* model) {
    reset();
    ...
    for (auto row = 0; row < model->rowCount(); ++row) {
        auto card_id = model->data(model->index(row, 0), CardListModel::IdRole).toString();
        auto counter = model->data(model->index(row, 0), CardListModel::Counter).toInt();

        cards_to_download.push_back(card_id);
        counters[card_id] = counter;
    }

    models_manager_->searchCardsByIdList(cards_to_download, [this, counters](CardListPtr cards) {
        if (!cards) {
            emit exportFinished();
        }
...
void DeckExporter::addCard(const CardPtr& card, int counter) {
    QString line = QString("%1 %2 %3 %4").arg(counter).arg(card->name()).arg(card->ptcgoCode()).arg(card->cardNumber());
