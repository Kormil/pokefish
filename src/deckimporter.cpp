...
int
DeckImporter::loadData(const QString& text)
{
...
int
DeckImporter::loadDataFromFile(const QString& file_url)
{
  QFile file(file_url);
...
DeckImporter::start()
{
  ...
  for (auto& card : cards_) {
    ...
    models_manager_->searchCardsByName(
      parameters,
      [this, &card](CardListPtr cards) {
        if (!cards || !cards->size()) {
          qWarning() << "No cards";
          ++download_errors_;
...
std::vector<DeckImporter::ImportedCard>
DeckImporter::parseImportedDeck(const QStringList& imported)
{
  ...
  for (const auto& line : imported) {
    ...
    if (parseLineFromDeck(line, card)) {
      qDebug() << card.counter << " " << card.card_name << " " << card.series
               << " " << card.card_number;
      ret.push_back(card);
    }
...
bool
DeckImporter::parseLineFromDeck(const QString& line, ImportedCard& parsed_line)
{
...
DeckImporter::ImportedCard
DeckImporter::parseEnergy(int counter,
                          QString name,
                          QString series,
                          QString card_number)
{
  if (series.contains("energy", Qt::CaseInsensitive)) {
    series = "SUM";
    card_number =
      QString::number(FIRST_ENERGY + (card_number.toInt() % ENERGY_CARDS));
  }
...
void
DeckImporter::setModelsManager(ModelsManager* modelsManager)
{
  models_manager_ = modelsManager;
}

int
DeckImporter::alreadyDownloaded()
{
  return already_downloaded;
}

QAbstractListModel*
DeckImporter::importedCards()
{
  return card_list_model_.get();
}

int
DeckImporter::downloadErrors()
{
