...
bool
CardListProxyModel::lessThan(const QModelIndex& source_left,
                             const QModelIndex& source_right) const
{
  ...
  if (m_sortedBy >= SortCards::BySupertype) {
    ...
    int result =
      QString::localeAwareCompare(leftData.toString(), rightData.toString());
    if (result != 0) {
      return result < 0;
    }
  }
...
QVariant
CardListProxyModel::data(const QModelIndex& index, int role) const
{
...
CardListProxyModel::roleNames() const
{
  auto model = sourceModel();
  if (!model) {
...
int
CardListProxyModel::rowCount(const QModelIndex& parent) const
{
...
SortCards::EnSortCards
CardListProxyModel::sortedBy() const
{
  return m_sortedBy;
}

void
CardListProxyModel::setSortedBy(const SortCards::EnSortCards& sortedBy)
{
  m_sortedBy = sortedBy;
  invalidate();
}
...
QAbstractListModel*
CardListProxyModel::cardListModel() const
{
  return m_cardListModel;
}

void
CardListProxyModel::setCardListModel(QAbstractListModel* cardListModel)
{
...
bool
CardListProxyModel::sorting() const
{
  return m_sorting;
}

void
CardListProxyModel::setSorting(bool sorting)
{
  m_sorting = sorting;
  invalidateFilter();
}
...
int
CardListModel::rowCount(const QModelIndex& parent) const
{
...
QVariant
CardListModel::data(const QModelIndex& index, int role) const
{
...
void
CardListModel::setCardList(CardListPtr cards)
{
  beginResetModel();
...
Card*
CardListModel::getRaw(QString id)
{
  if (!m_cards) {
    return nullptr;
  }
...
Q_INVOKABLE void
CardListModel::append(QVariantMap obj)
{
...
void
CardListModel::append(CardPtr card, int count)
{
  if (!m_cards || !card) {
    return;
  }
...
void
CardListModel::appendList(CardListPtr cards)
{
  if (!m_cards || !cards) {
    return;
  }
